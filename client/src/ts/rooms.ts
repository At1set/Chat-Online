...
import {
  mapRoomDTO,
  mapRoomsPollingResponseDTO,
} from "./modules/Api/mapping.js"
import { PollingResponseAction, Room } from "./modules/Api/types.js"
import { room } from "./modules/templates.js"

window.onload = () => {
  const login = new URLSearchParams(window.location.search).get(
    "user_name"
  )! as string
  ...
  profile_name.innerHTML = login as string
  ...
  const privateCheckBox: HTMLInputElement = document.querySelector("#private")!
  ...
  createRoomFromElement.addEventListener("submit", (e) => {
    ...
    if (!name || (privateCheckBox.checked && !password)) return
    const sendingData = {
      name,
      author: login,
      password,
    ...
    Api.createRoom(sendingData).then((res) => {
      if (res.error) alert(JSON.stringify(res.error))
        ...
        if (password) newUrlSearch.append("password", password as string)
  ...
  modal_window.addEventListener("click", (e) => {
    if (!selectedRoom) return modal_window.classList.remove("_active")
    const target = e.target as HTMLElement
    if (target.closest(".modal__body")) return
    else modal_window.classList.remove("_active")
  })
  ...
  modal_window.querySelector("form")!.addEventListener("submit", (e) => {
    ...
    const password = formData.get("room-password") as string
    if (!password) return
    Api.connectToRoom(selectedRoom.id, login, password).then((res) => {
      if (res.error) {
        const submit_button = form.querySelector("button")! as HTMLButtonElement
