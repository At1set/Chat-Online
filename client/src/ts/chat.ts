...
import {
  mapMessageDTO,
  mapRoomPollingResponseDTO,
} from "./modules/Api/mapping.js"
import { Message } from "./modules/Api/types.js"
import { message } from "./modules/templates.js"

window.onload = () => {
  const search_params = new URLSearchParams(window.location.search)
  const room_id = window.location.pathname.split("/").pop() as string
  if (!room_id || Number.isNaN(room_id)) window.location.href = "/rooms"
  ...
  const sending_form: HTMLFormElement =
    document.querySelector(".chat__input form")!
  const back_button = document.querySelector(".chat__back button")!

  function appendMessage(messageData: Message) {
  ...
  const pollerObject = Api.startRoomPolling(
    login,
    room_id,
    password,
    (data) => {
  ...
  sending_form.addEventListener("submit", (e) => {
    ...
    const message_content: string | null = (
